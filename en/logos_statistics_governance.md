...
From an engineering standpoint, LLMs are the ultimate empirical validation of Wittgenstein’s thesis that "meaning is use." When we train a model to build a High-Dimensional Vector Space (Embeddings), we are essentially translating "language games" into geometry. A vector does not encode the logical truth of a concept, but the statistical coordinates of its usage. The model predicts the next token simply by calculating the distance between these usages in vector space, perfectly mimicking the rules of the game without ever playing it in reality.

But here lies the problem of Generative AI. LLMs can generate the phrase "I will come the day after tomorrow" with perfect syntax. But, just like Wittgenstein's dog, they do not inhabit time. They have no Form of Life. For an AI, "the day after tomorrow" is not an expectation or a commitment to the future; it is just a statistical probability following the word "come."
